    ...
    public partial class FrmMain : Form
    {
        ...
        private void btn_Start_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int rowIDX = 0; rowIDX < l_getDataRow.Length; rowIDX++)
                {
                    ...
                    int StartGeneration = int.Parse(l_getDataColumn[4]);
                    //int EndGeneration = int.Parse(l_getDataColumn[5]);

                    int RunGeneration = StartGeneration;

                    //while (RunGeneration <= EndGeneration)
                    //{

                        string l_AML = "";
                        string l_tempAML = txt_SearchAML.Text.Trim();

                        l_tempAML = l_tempAML.Replace("$1", l_getDataColumn[0]);//style
                        l_tempAML = l_tempAML.Replace("$2", l_getDataColumn[3]);//version
                        l_tempAML = l_tempAML.Replace("$3", RunGeneration.ToString());//generation
                        l_tempAML = l_tempAML.Replace("$4", l_getDataColumn[1]);//plu
                        ...
                        for (int garmentIDX = 0; garmentIDX < l_GetItem.getItemCount(); garmentIDX++)
                        {
                            ...
                            TreeNode l_garmentStyle_Item_Node = new TreeNode(l_garmentStyle_Item.getProperty("item_number", "-") + "(" + l_garmentStyle_Item.getProperty("major_rev", "-") + "-" + l_garmentStyle_Item.getProperty("generation", "-") + ")", 0, 0);
                            ...
                            if (l_GarmentStyleContainsOption_Item != null && l_GarmentStyleContainsOption_Item.getItemCount() > 0)
                            {
                                for (int garOptIDX = 0; garOptIDX < l_GarmentStyleContainsOption_Item.getItemCount(); garOptIDX++)
                                {
                                    ...
                                    {
                                        Item l_GarmentBOM_Item = l_GarmentStyleOptionBOM.getItemByIndex(garOptBomIDX).getRelatedItem();
                                        ...
                                        {
                                            Item l_part_Item = l_GarmentBOMPart_Item.getItemByIndex(partIDX).getRelatedItem();
        ...
        private void btn_StartReplace_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DialogResult result = MessageBox.Show("Replace PLU Just Change Appoint Version And Generation, Do You Know That ?", "System Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
